...
import { fetchSingleMarket } from "../../redux/slices/marketSlice";
...
const SingleMarket = () => {
  ...
  const { singleMarket, loading, error } = useSelector(
    (state) => state.markets
  );
  ...
  if (loading || !singleMarket) return <Loading />;
  if (error) return <div>Error: {error}</div>;

  const { image, title, description } = singleMarket;
